...
class ChannelExtractor {
  ...
  static Future<YoutubeChannel> channelInfo(String url) async {
    if (url == null || StringChecker.hasWhiteSpace(url)) throw BadUrlException("Url is null or contains white space");
    Future<dynamic> task() => NewPipeExtractorDart.extractorChannel.invokeMethod('getChannel', {"channelUrl": url});
  ...
  static Future<List<StreamInfoItem>> getChannelUploads(String url) async {
    if (url == null || StringChecker.hasWhiteSpace(url)) throw BadUrlException("Url is null or contains white space");
    Future<dynamic> task() => NewPipeExtractorDart.extractorChannel.invokeMethod('getChannelUploads', {"channelUrl": url});
    var info = await task();
    // Check if we got reCaptcha needed response
  ...
  static Future<String> getAvatarUrl(String channelId) async {
    ...
    return parser.parse(raw).querySelector('meta[property="og:image"]')?.attributes['content'];
  }
}
